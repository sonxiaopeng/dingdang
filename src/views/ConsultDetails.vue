<template>
	<div id="consult-detail">
		...
		<img v-else src="/img/NO_DATA.png" id="no-data" :style="{height: '200px', width: '200px', margin: '100px auto', display: 'block'}" />
	</div>
</template>
<style scoped>
#consult-detail {
...
export default {
	...
	mounted() {
		...
		//添加获取文章事件
		this.axios
			.get(`/yyy?disease_id=${this.$store.state.disease_id}`)
			.then(res => {
				// 判断是否有文章数据
				if (res.data.code == 1) {
					// 没有数据不显示页面
